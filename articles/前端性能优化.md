...
同时这不仅可以作为懒加载的手段，也可以修改以下作为一个曝光打点的组件。关键在个人怎么使用。
...
我写了一篇[长列表优化方案解析][长列表优化方案解析.md]，里面详细解释了通过js计算节点高度做的长列表优化手段。
...
参考👉[Resource Hints介绍.md](./Resource%20Hints%E4%BB%8B%E7%BB%8D.md)

### 组件渲染异常兜底

在日常开发中，应该遇见过“一个数据结构不对导致页面空白”的问题，往往这种锅是可以丢给后端同事的，但是并不是说前端毫无责任，因为页面空白是可以避免的，只是你没做而已。
...
如果页面中有类似scroll/resize这类的需求的话，建议还是加上节流或者防抖。
...
最后，或许react新推出的hooks也可以起到全局状态管理的作用，这块后面再补上响应的文章详细介绍以下吧！
...
> 其实性能优化（不仅是前端）无非就是几点，“并行（并发），预加载（预渲染），懒加载（分包），复用（实例），缓存（数据/文件），压缩（代码/数据）”，从每一点入手，深入优化，多问问自己还能做什么，性能自然就上去了。
...
[Resource Hints介绍](./Resource%20Hints%E4%BB%8B%E7%BB%8D.md)
